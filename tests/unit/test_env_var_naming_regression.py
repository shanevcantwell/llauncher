...
The allowlist is the set of paths where matches are *expected*:
    ...
    - ``docs/generated/TEST_SUITE_SUMMARY.md`` is generated by
      ``scripts/summarize_tests.py`` and mirrors test docstrings
      verbatim — including the ones in *this* category that name the
      legacy variable to assert it is rejected. The summary is an
      envelope over already-allowlisted test source, not a fresh
      functional reference, so it is allowlisted in kind.
...
ALLOWED_PATH_PREFIXES: tuple[str, ...] = (
    ...
    "docs/generated/TEST_SUITE_SUMMARY.md",
