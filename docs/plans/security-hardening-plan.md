...
2. **MCP HTTP transport plans?** If the MCP server ever grows an HTTP transport (the MCP spec supports it), this plan's "MCP is stdio-only, trust-the-launcher" stance collapses. Out of scope for now; flag if anyone proposes adding HTTP MCP.
3. **`LAUNCHER_MODELS_ROOT` scope** — does the operator want a hard root, or just a soft warning when paths escape it? The plan proposes hard validation on save (errors surface in UI), but a soft warning would break less existing data. **Operator answer needed before C8 is filed.**
4. **Streamlit dashboard auth** — currently relies on the operator binding to localhost. Is there appetite for a basic-auth layer on the dashboard itself? Plan says no (over-engineering for the threat model); flagging in case the operator disagrees.
