...
| `llauncher/state.py` | 01 only | Add behavioral change comment/docstring to `start_server()` noting ADR-LLNCH-005 pre-flight health gate replacing old Path.exists() check. Pure doc addition — zero risk. |
...
├─ ADR-LLNCH-003: alternatives analysis (mTLS/OAuth/unix-socket/reverse-proxy), consequences/risk tables, Typer dependency gap documentation (Plans 03+06)
├─ ADR-LLNCH-004: build-vs-deploy CLI framework analysis, dependency gap fix recommendation, double-discovery problem treatment (Plan 06)
├─ ADR-LLNCH-005: two-layer health check decision justification, 1MiB heuristic rationale, GGUF header validation deferred (Plan 06)
└─ ADR-LLNCH-006: /dev/memfd fabrication removed, build-vs-adopt nvidia-smi analysis, honest per-platform accuracy statements (Plans 04/05 + Plan 06)
...
| I | ADR Documentation Rewrites | 8 | No | 1 (concurrent with G — different skill set) | Doc-only changes. Can be merged separately or batched into the main PR. If factual error introduced: revert individual ADR file. | None. Pure documentation work that can begin immediately after Phase A (which produces the verification gate checklist and cross-ref structure). Overlaps entirely with phases B through G wall-clock time. | ADR-LLNCH-003 rewrite (~2h): alternatives analysis mTLS/OAuth/unix-socket/reverse-proxy, consequences/risk tables, Typer dependency gap documentation. ADR-LLNCH-004 rewrite (~2h): CLI framework alternatives (Click/argparse/plain-python), pyproject.toml dependency gap fix recommendation, double-discovery problem treatment. ADR-LLNCH-005 rewrite (~2h): two-layer health check decision, 1MiB heuristic rationale, GGUF deferred analysis, open questions. ADR-LLNCH-006 rewrite (~2h): /dev/memfd fabrication removed, build-vs-adopt analysis, honest per-platform accuracy statements. Consolidated: ~8 hrs |
