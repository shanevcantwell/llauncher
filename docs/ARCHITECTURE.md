...
| Ecosystem ground-physics constitution | Rules 4, 5, 6 — ONE-MINT, IDENTITY⊥ENVELOPE, EMIT-CANONICAL, PARSE-AT-THE-DOOR | `harness-tools` → `docs/ground-physics/GROUND_PHYSICS.md` (six data-plane invariants + dev-plane disciplines; migrated 2026-06-22 from the design-docs scratchpad). The earlier "out-of-tree (private) / restated in `CLAUDE.md`" note was stale — no such restatement existed. |
