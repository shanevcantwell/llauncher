...
|-----|-------|----------------|------------|
| 002 | Unified Swap-with-Eviction Semantics | **Superseded by ADR-011** | N/A (supersedes) |
...
**Status:** ❌ **Superseded by ADR-011 before implementation.**

The ADR was marked as "Superseded by ADR-011" in its header (2026-04-25), but no code implementing the full 5-phase rollback design exists in `llauncher/state.py`. The current `state._start_with_eviction_impl()` method implements a simplified version without:
- In-flight marker (`{port}.swap`) for concurrent-swap rejection
- Lazy reconciliation of stale lockfiles/markers  
- Full pre-flight VRAM checks (ADR-006)
- All the response fields specified in ADR-011's `SwapResult`

**Compliance:** N/A - superseded without implementation.

---

### ADR-003: Authentication for Agent API
...
**Violations:**
1. **Missing port argument validation:** ADR-010 requires `port` to be a required parameter at the API boundary, but CLI's `start_server()` accepts optional `--port` with fallback to env var (permissive). No error if neither provided.
   - File: `cli.py:178-192`
   
2. **Missing swap command:** ADR-010 establishes `/swap/{port}` as a distinct verb, but CLI has no `llauncher server swap` subcommand.
...
**Violations:**
1. **No `/models/health` endpoint:** ADR-005 specifies API endpoints but none exist in the agent.
...
### ADR-006: GPU Resource Monitoring
...
**Violations:**
1. **No `/status?full=true` endpoint:** The agent exposes `/status` but does NOT extend it with GPU data per ADR-006.

2. **Pre-flight VRAM check not implemented in swap:** ADR-011 requires VRAM headroom checks, and the v2 operations layer accepts a `vram_check` callable parameter—but no implementation wires this up (see `operations.py:487+`).
...
| Component | File | Status |
|-----------|------|--------|
| Lockfile module | `llauncher/core/lockfile.py` | ✅ Full implementation per ADR-008 |
| Marker module | `llauncher/core/marker.py` | ✅ In-flight swap marker (ADR-011) |
| Audit log | `llauncher/core/audit_log.py` | ⚠️ Only stub exists |

**Violations:**
1. **No audit log implementation:** ADR-008 requires persisted JSON Lines at configurable path, but `llauncher/core/audit_log.py` is a minimal stub with no write operations.

2. **Stateful `LauncherState.refresh()` still present:** File `state.py:63-74` shows `refresh()` method exists and reloads from disk—but per ADR-008 it should be removed entirely (stateless facade).
...
### ADR-010: Port Ownership at Call Site
...
**Violations:**
1. **Legacy fallback in CLI still present:** ADR-010 requires port as required parameter, but `cli.py:186-204` allows missing port with env var fallback:
   ```python
...
2. **Legacy v1 state path still exists:** `state.py:345-407` (`start_server()`) contains auto-allocation logic for when port=None—per ADR-010, this should be removed from the API layer (CLI may implement opt-in).
...
| Component | File | Status |
|-----------|------|--------|
| Operations layer swap() | `llauncher/operations.py:361-608` | ✅ Full implementation per ADR-011 |
| In-flight marker | `llauncher/core/marker.py` | ✅ Atomic O_EXCL creation, stale reconciliation |

**Violations:**
1. **Legacy v1 swap in MCP tools:** File `mcp_server/tools/servers.py:495+` still uses `state._start_with_eviction_impl()` which implements ADR-002 semantics (not ADR-011's marker-based concurrency control).

2. **No VRAM pre-flight hook:** ADR-011 requires VRAM checks, but the swap implementation accepts optional checkers and none are wired up in production code.

3. **Startup logs cap mismatch:** ADR-011 open question 2 specifies 100 lines; `operations.py:26` sets `STARTUP_LOG_TAIL_MAX = 100` ✅, but the v1 path in `state.py` does not return this field.
...
| Gap | Impact |
|-----|--------|
| No `/models/health` endpoint (ADR-005) | Operators cannot discover missing/corrupted model files before starting servers |
| No GPU metrics in `/status` (ADR-006) | Cannot make informed scheduling decisions; OOM crashes still possible |
| Audit log not persisted (ADR-008) | Governance and debugging signals lost on restart |
| Legacy v1 swap path remains (ADR-011) | Concurrent-swap rejection only works in new operations layer, not MCP tools |

---

## Recommendations

1. **Remove legacy v1 code paths** that violate ADR-010 port ownership:
...
3. **Implement audit log persistence** per ADR-008:
   - Create JSON Lines writer at configurable path
   - Distinguish commanded vs observed events

4. **Add `/models/health` endpoint** per ADR-005 to expose model file status via HTTP Agent.
