...
5. **Canonical self-swap worked example** — operationalize ADR-LLNCH-011's claims as an integration test.
...
## Item 1 — Footer Contract (ADR-LLNCH-012)
...
## Item 2 — Logs Lifecycle (ADR-LLNCH-013)
...
- Add `LAUNCHER_LOG_DIR` env override (paired with the existing `LAUNCHER_RUN_DIR` and `LAUNCHER_AUDIT_PATH` per ADR-LLNCH-008).
...
Spike §5 noted a name-sanitization collision risk (two configs sanitize to the same filename). Out of scope for ADR-LLNCH-013 (it's a config validation problem, not a logs problem) — file as a separate Issue and let ADR-LLNCH-013 reference it.

## Item 3 — Cancellation (ADR-LLNCH-014)
...
The in-flight marker (ADR-LLNCH-011) is the cancel signal. A `cancel: true` flag written into `{port}.swap` is checked at every readiness poll iteration:
...
## Item 4 — Orphan Policy (ADR-LLNCH-015)
...
## Item 5 — Canonical Self-Swap Example (ADR-LLNCH-016)

### Problem

ADR-LLNCH-011 claims "swap leaves the inference channel intact for the harness's MCP transport." The mechanism is structurally true (separate stdio process for MCP, separate HTTP/HTTPS for inference), but it's not documented anywhere and not exercised by a test that proves end-to-end correctness.

### Decision Sketch

This one is mostly documentation and one big integration test:

- ADR-LLNCH-016 captures the worked example in prose: "the harness sends `swap_server(port=P, model_name=B)` over MCP; here's the timeline of what dies and what stays alive; here's the response shape; here's what the harness does on each `port_state` value."
...
1. **Footer contract — should it support batch?** `GET /footer-context?ports=8081,8082`. Probably yes; the footer often watches more than one port. Decide in ADR-LLNCH-012.
...
3. **Cancellation — should the marker carry an opaque token?** I.e., `cancel_token: <uuid>` so a stale cancel doesn't interrupt the next swap. Probably yes; design in ADR-LLNCH-014.
...
- ADR-LLNCH-008 — lockfile + audit log (orphan + cancellation depend on this)
- ADR-LLNCH-011 — in-flight marker (cancellation lives here)
- ADR-LLNCH-001 — TypeScript footer extension (item 1's consumer)
