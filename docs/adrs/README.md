...
- [ADR-027 — Model Validate — a Single Read-Only Validation Path Reused Everywhere](./draft/027-model-validate-read-only-verb.md) — ratified and implemented end-to-end in the same PR (#475); file kept under `draft/` (its original path from the ratification comment) rather than moved — no open gaps tracked
