...
- **ADR-LLNCH-012 (Footer Context Endpoint)** is the *static* sibling: it serves model/ctx_size/parallel from the lockfile + ConfigStore and **deliberately never contacts the live server**. ADR-LLNCH-019 supplies the *live* telemetry ADR-LLNCH-012 explicitly excludes. They **compose** — static context + live activity = the full footer view. ADR-LLNCH-019 does not modify ADR-LLNCH-012's pinned `/footer-context/{port}` contract.
- **ADR-LLNCH-006 (GPU Resource Monitoring)** is the collector-pattern precedent: `core/server_metrics.py` is a peer to `core/gpu.py` (stateless collector, short TTL cache, degraded envelope, injectable backend seam).
- **ADR-LLNCH-003 (Agent API Authentication)** governs the sensitive endpoint's auth (matches `/status`, no exemption).
- **ADR-LLNCH-009 (Hub/Spoke)** — fleet aggregation is the `remote`/UI path, not MCP. **ADR-LLNCH-010** — endpoints are port-keyed.
...
| Hardware | `nvidia-smi` | ADR-LLNCH-006 (exists) |
...
**Why ADR-LLNCH-012 doesn't cover it.** ADR-LLNCH-012's endpoint is cheap *because* it never touches the live server (lockfile + config only). Live activity (tok/s, slot phase) requires contacting the model server, a different cost class — folding it into `/footer-context/{port}` would break that endpoint's pinned, probe-free premise. Hence a separate surface.

## Decision

### 1. `core/server_metrics.py` — a stateless, point-in-time reader (peer to `core/gpu.py`)

- **Stateless per call.** Each read polls the target model server over localhost HTTP and returns a snapshot. No cross-call accumulation (rule 3). A short TTL cache (default ~2 s, `LLAUNCHER_METRICS_CACHE_S`) absorbs poll cadence, mirroring ADR-LLNCH-006's collector and ADR-LLNCH-012's footer cache.
...
Sensitivity is enforced by *which method/endpoint/tool* you call, never by a `include_prompts=true` argument on a shared call. `started_at` is sourced from the lockfile (the field ADR-LLNCH-012 deferred) and folded into the aggregate payload — a coarse uptime/timer at no extra probe cost.
...
### 5. Agent endpoints — port-keyed, pinned, auth = `/status` (ADR-LLNCH-012 DNA)
...
- **Node = the connection target** (the agent serves its own node); no node in the path, consistent with ADR-LLNCH-010/-012.
...
- Response shapes are **pinned by this ADR** (ADR-LLNCH-012 precedent): extend only by amendment.
...
- **MCP is local-node by design** (stdio, this node only), like `server_status`/`get_server_logs`. Fleet monitoring is the `remote`/UI path (ADR-LLNCH-009), not MCP — stated explicitly so a port-keyed MCP tool is never read as fleet-wide.
...
### 8. Composition with ADR-LLNCH-012

The footer's full view = ADR-LLNCH-012 `/footer-context/{port}` (model, ctx_size, parallel — static) **+** ADR-LLNCH-019 `/server-metrics/{port}` (phase, tok/s — live). Two cheap, pinned, port-keyed endpoints on the same agent; the consumer composes them.
...
### Positive
- Activity is visible without log-tailing; the footer can return to consuming llauncher (its original ADR-LLNCH-012-era intent) instead of polling model servers directly.
...
- **`started_at` included.** The aggregate payload carries `started_at` (from the lockfile — the field ADR-LLNCH-012 deferred), giving the coarse uptime/timer both consumers want.
- **No batch variant.** `/server-metrics?ports=…` is rejected (ADR-LLNCH-012 §5 reasoning: one occupied port is typical, the TTL absorbs the cost). Revisit only if a real N-port-per-node cost surfaces.

## Relation to the decision record

| ADR / decision | Relationship |
|---|---|
| ADR-LLNCH-012 footer-context | static sibling; ADR-LLNCH-019 adds the live telemetry it excludes; composes; does not alter its pinned contract |
| ADR-LLNCH-006 gpu monitoring | collector pattern (`core/server_metrics.py` ⟷ `core/gpu.py`) |
| ADR-LLNCH-003 agent auth | governs the sensitive slot endpoint |
| ADR-LLNCH-009 hub/spoke | fleet path is remote/UI, not MCP |
| ADR-LLNCH-010 port ownership | endpoints port-keyed |
