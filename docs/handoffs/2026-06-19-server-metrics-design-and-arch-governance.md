...
A "monitor llama-server instances" feature request became three things: (a) llauncher's first governance-grade **`docs/ARCHITECTURE.md`**, (b) a ratified **ADR-019** defining the metrics *surface*, and (c) several reconciliations/boundary corrections. **No monitoring code was written** — ADR-019 implementation is the next step (`auto:fix`).
...
| #178 | **ADR-019** (`docs/adrs/accepted/019-server-metrics-surface.md`) — the metrics surface. |
...
## The monitoring design (ADR-019 is the durable spec)

Third monitoring layer = **live in-server inference telemetry** (alongside process-liveness and ADR-006 GPU metrics). Shape:
...
- **Agent endpoints:** `GET /server-metrics/{port}` + `/server-slots/{port}` — port-keyed, pinned, auth = `/status`. **Composes with ADR-012** (`/footer-context/{port}` static context + live activity = full footer view).
...
- The shipped footer drifted to polling model servers directly; routing via :8765 (ADR-012 + ADR-019) is a **return to the original requirements intent**.

## Next step

Implement ADR-019 — tracked as **#179** (`auto:fix`). Scope, test plan, gates, and the in-code shortcut-note requirement (#174/#175/#176) live in the issue, not here (docs convention: dossiers document design; tracked work lives in GH Issues).
