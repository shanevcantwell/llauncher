...
Read-only validation of configured model weights (issue #475, ADR-027). Checks file existence/readability/size and GGUF magic bytes (both gate the result); VRAM headroom and lockfile staleness are reported as advisory (never gate the result, and validation never reconciles a stale lockfile). Never starts a process, deletes a config entry, or writes an audit line — the read-only companion to `delete_model`.
