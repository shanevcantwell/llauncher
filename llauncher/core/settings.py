...
# auto-allocation fallback (ADR-LLNCH-010 / issue #58 require explicit ports at
...
# Lockfile directory for running servers (per ADR-LLNCH-008).
...
# Audit log path (per ADR-LLNCH-008). JSON Lines, append-only.
...
# Per-server log directory (ADR-LLNCH-013). Files inside are
...
# Size cap for a single live log file before rotation kicks in (ADR-LLNCH-013).
...
# (ADR-LLNCH-013). With the default 3, the on-disk set is
...
# (ADR-LLNCH-012). Default 1.0 absorbs footer poll cadence (multiple
